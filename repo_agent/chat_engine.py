...
class ChatEngine:
    ...
    def reduce_input_length(self, shorten_attempt, prompt_data):
        ...
        # 按照academic的策略，使用1/16进行逐次截断
        refer_cut_len = len(prompt_data.referencer_content) * (1 - shorten_attempt // 16)
        refed_cut_len = len(prompt_data.reference_letter) * (1 - shorten_attempt // 16)
        prompt_data.referencer_content = prompt_data.referencer_content[:refer_cut_len]
        prompt_data.reference_letter = prompt_data.reference_letter[:refed_cut_len]

        # 原始阶段策略是先删除project_structure和project_structure_prefix，然后再删除referencer和referenced信息
    ...
    def format_code(self, content):
        formatted_code = autopep8.fix_code(content, options={"max_line_length": 9999})
        cleaned_code = []
        # 只去掉单行#注释
    ...
    def generate_doc(self, doc_item: DocItem, file_handler):
        ...
        code_content = self.format_code(code_content)
        ...
        project_structure = self.project_manager.build_path_tree(who_reference_me, reference_who, doc_item_path)
        ...
        total_tokens = self.num_tokens_from_string(sys_prompt) + self.num_tokens_from_string(usr_prompt)

        # 如果总tokens超过当前模型的限制，则尝试寻找较大模型或者缩减输入
        if total_tokens >= max_input_length:
            # 查找一个拥有更大输入限制的模型
            larger_models = {
                k: v for k, v in max_input_tokens_map.items() if (v - max_tokens) > total_tokens
            ...
            for shorten_attempt in range(1, 17):
